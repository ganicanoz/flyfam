-- Havada / taksi / kalkış sonrası: ETD ileri gitse bile faz ACTIVE kalsın; stale (STA+3h) uzun uçuşu passive_past yapmasın.

drop function if exists public.compute_flight_api_phase_state(
...
begin
  ...
  -- Uçak hareket halindeyken zaman penceresi / ETD oynaması fazı düşürmesin (iniş sinyaline kadar).
...
comment on function public.compute_flight_api_phase_state(
  ...
  'api_refresh_phase: landed→past; taxi_out/departed/en_route→active+lock; else ETD−30m / stale / STD−3h rules.';
...
begin
  ...
  v_airborne :=
    coalesce(new.flight_status, '') in ('taxi_out', 'departed', 'en_route')
    or coalesce(new.internal_status, '') in ('taxi_out', 'departed', 'en_route');
...
comment on column public.flights.phase_active_locked is
  'ACTIVE veya havada (taxi_out/departed/en_route) iken kilit; ETD oynaması fazı düşürmez, inişe kadar.';
