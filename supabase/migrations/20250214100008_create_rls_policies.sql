...
drop policy if exists "Crew can read own crew_profile" on public.crew_profiles;
create policy "Crew can read own crew_profile"
  on public.crew_profiles for select
  using (auth.uid() = user_id);

drop policy if exists "Crew can insert own crew_profile" on public.crew_profiles;
create policy "Crew can insert own crew_profile"
  on public.crew_profiles for insert
  with check (auth.uid() = user_id);

drop policy if exists "Crew can update own crew_profile" on public.crew_profiles;
create policy "Crew can update own crew_profile"
  on public.crew_profiles for update
  using (auth.uid() = user_id);

drop policy if exists "Crew can delete own crew_profile" on public.crew_profiles;
...
drop policy if exists "Crew can manage connections where they are crew" on public.family_connections;
...
drop policy if exists "Family can read own connections" on public.family_connections;
create policy "Family can read own connections"
  on public.family_connections for select
  using (family_id = auth.uid());

drop policy if exists "Family can insert connection (request to connect)" on public.family_connections;
...
drop policy if exists "Crew can manage own invite codes" on public.invite_codes;
...
drop policy if exists "Anyone can read invite codes (for lookup by code)" on public.invite_codes;
