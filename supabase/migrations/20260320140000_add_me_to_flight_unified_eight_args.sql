-- Tek imza: 8 argüman (son ikisinde default). PostgREST + istemci uyumu.
-- Önceki migration'larda 6 ve 8 parametreli overload'lar birikebiliyor; hepsini kaldırıp tek tanım bırakıyoruz.

drop function if exists public.add_me_to_flight(text, date, text, text, timestamptz, timestamptz);
...
comment on function public.add_me_to_flight(text, date, text, text, timestamptz, timestamptz, text, timestamptz) is
  'Find flight by number+date or create; add crew to flight_crew. Merges missing schedule/airports. duty_off/sim: roster_entry_kind + duty_rest_end.';

grant execute on function public.add_me_to_flight(text, date, text, text, timestamptz, timestamptz, text, timestamptz) to authenticated;
grant execute on function public.add_me_to_flight(text, date, text, text, timestamptz, timestamptz, text, timestamptz) to service_role;
