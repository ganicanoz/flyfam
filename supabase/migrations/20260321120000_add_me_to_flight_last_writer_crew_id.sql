-- Global uçuş satırında `crew_id`: son `add_me_to_flight` çağıran (meta). Plan/rota zaten tüm crew için ortak güncellenir.
...
comment on function public.add_me_to_flight(text, date, text, text, timestamptz, timestamptz, text, timestamptz) is
  'Find flight by number+date or create; add crew. Non-null p_* overwrites schedule/airports; crew_id = last caller.';
