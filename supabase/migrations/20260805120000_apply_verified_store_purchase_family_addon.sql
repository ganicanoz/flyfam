-- Family add-on IAP: product `03` (and aliases) increments extra_family_slots by 1.
...
begin
  ...
  if v_is_addon then
    ...
    -- Claim this transaction once (idempotent retries).
...
comment on function public.apply_verified_store_purchase(text, text, text, text, timestamptz, jsonb, timestamptz, boolean) is
  'Applies verified store purchase: base (01/02) or family add-on consumable (+1 extra_family_slots).';
