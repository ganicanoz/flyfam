-- Mobil PDF import `p_roster_detail` ile çağırır; 20260321120000 yalnızca 8 argümanlı imza bırakmıştı.
-- PostgREST: "Could not find the function public.add_me_to_flight(..., p_roster_detail, ...) in the schema cache"
...
comment on function public.add_me_to_flight(text, date, text, text, timestamptz, timestamptz, text, timestamptz, text) is
  'Find flight by number+date or create; add crew. Non-null p_* overwrite; crew_id = last caller; roster_detail for IndiGo PDF.';
