-- Accept ASC consumable EFUC (repeatable +1 family slot).
...
begin
  ...
  if upper(v_product) in ('EFU', 'EFUC') then
    v_product := upper(v_product);
  end if;
  ...
  if v_is_addon then
    ...
    if coalesce(v_extra, 0) >= v_max_extra then
      raise exception 'Extra family slot limit reached';
    end if;
...
comment on function public.apply_verified_store_purchase(text, text, text, text, timestamptz, jsonb, timestamptz, boolean) is
  'Applies verified store purchase: base (01/02) or family add-on EFUC/EFU (+1 extra_family_slots per tx).';
