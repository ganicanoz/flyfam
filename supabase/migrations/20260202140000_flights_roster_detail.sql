-- IndiGo PDF: Training Details ve benzeri opsiyonel roster notu (yalnızca import ile dolar).

alter table public.flights
  add column if not exists roster_detail text;

comment on column public.flights.roster_detail is
  'Opsiyonel roster açıklaması (ör. IndiGo Training Details kurs adı).';
...
comment on function public.add_me_to_flight(text, date, text, text, timestamptz, timestamptz, text, timestamptz, text) is
  'Find flight by number+date or create; add crew. roster_detail: IndiGo PDF Training Details vb.';

grant execute on function public.add_me_to_flight(text, date, text, text, timestamptz, timestamptz, text, timestamptz, text) to authenticated;
grant execute on function public.add_me_to_flight(text, date, text, text, timestamptz, timestamptz, text, timestamptz, text) to service_role;
